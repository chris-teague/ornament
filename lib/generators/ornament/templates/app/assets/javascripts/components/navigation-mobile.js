  ...
  $(document).on("ornament:refresh", function () {
    ...
    window.mobileNav = {
      ...
      updateMenuHeight: function(){
        ...
        // var mobileNav.scrollableLists = true;
        // if(mobileNav.scrollableLists) {
        //   var paneTop = 
        // }

        // v2
        var $currentTab = mobileNav.getCurrentTab();
        var trayHeight = mobileNav.tray.outerHeight();
        var $target = mobileNav.tray.find("." + mobileNav.firstPaneClass);

        if(mobileNav.currentLevel == 1) {
          $target = mobileNav.tray.find("." + mobileNav.firstPaneClass).find("ul").first();
        } else if ( $currentTab.is("a") ) {
          $target = $currentTab.parent("li").parent("ul");
        } else if ( $currentTab.children("div").length ) {
          $target = $currentTab.children("div").children("ul");
        }

        // Include height for last border
        trayHeight = trayHeight + 1;

        $target.height(trayHeight);
      ...
      resetPaneHeights: function(){
        // Fix flickering when coming back to a differently sized pane
        // by resetting heights on other panes
        var scrollableRegions = mobileNav.tray.find("ul").not("."+mobileNav.nonPaneClass);
        // mobileNav.scrollableRegions.height("auto");
      },
